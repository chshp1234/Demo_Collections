...
/**
 * ================================================ 作
 * 者：jeasonlzy（廖子尧）Github地址：https://github.com/jeasonlzy 版 本：1.0 创建日期：16/9/11 描 述：Https相关的工具类 修订历史：
 * ================================================
 */
public class HttpsUtils {
    ...
    /** https单向认证 可以额外配置信任服务端的证书策略，否则默认是按CA证书去验证的，若不是CA可信任的证书，则无法通过验证 */
    ...
    /** https双向认证 bksFile 和 password -> 客户端使用bks证书校验服务端证书 certificates -> 用含有服务端公钥的证书校验服务端证书 */
    public static SSLParams getSslSocketFactory(
    ...
    /**
     * https双向认证 bksFile 和 password -> 客户端使用bks证书校验服务端证书 X509TrustManager ->
     * 如果需要自己校验，那么可以自己实现相关校验，如果不需要自己校验，那么传null即可
     */
    public static SSLParams getSslSocketFactory(
            InputStream bksFile, String password, X509TrustManager trustManager) {
        ...
        try {
            ...
            TrustManagerFactory tmf =
                    TrustManagerFactory.getInstance(
                            /*通过信任管理器获取一个默认的算法*/ TrustManagerFactory.getDefaultAlgorithm());
     ...
     * 则验证机制可以回调此接口的实现程序来确定是否应该允许此连接。策略可以是基于证书的或依赖于其他验证方案。 当验证 URL
     * 主机名使用的默认规则失败时使用这些回调。如果主机名是可接受的，则返回 true
     */
