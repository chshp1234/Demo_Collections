...
public class AudioRecordUtils {
    ...
    private static AudioRecordUtils result;

    public static AudioRecordUtils getInstance(Boolean recordingCompressed) {
        ...
        if (recordingCompressed) {
            ...
            do {
                result =
                    ...
                    & !(result.getState() == AudioRecordUtils.State.INITIALIZING));
        }
        return result;
    }
    ...
    /**
     * Returns the state of the recorder in a RehearsalAudioRecord.State typed object. Useful, as no
    ...
    private AudioRecord.OnRecordPositionUpdateListener updateListener =
            new AudioRecord.OnRecordPositionUpdateListener() {
                public void onPeriodicNotification(AudioRecord recorder) {
                    ...
                    Log.d("updateListener", "recording is ing");

                    try {
                        randomAccessWriter.write(buffer); // Write buffer to file
                        Log.d("updateListener", "file size:" + randomAccessWriter.length());
                        ...
                        if (bSamples == 16) {
                            for (int i = 0; i < buffer.length / 2; i++) { // 16bit
                                ...
                                //						Log.e(TAG, "updateListener 16bit curSample",
                                //								curSample + "");
                        ...
                        //				Log.e(AudioRecordUtils.class.getName(),
                        //						"Error occured in updateListener, recording is aborted");

                        Log.d("updateListener", "record succ");
                        // stop();
                        reset();
                    }
                }
    ...
    public AudioRecordUtils(
        ...
        try {
            ...
            if (rUncompressed) { // RECORDING_UNCOMPRESSED
                ...
                if (bufferSize
                        < AudioRecord.getMinBufferSize(
                                sampleRate, channelConfig, audioFormat)) { // Check to make sure
                    // buffer size is not
                    // smaller than the
            ...
            if (e.getMessage() != null) {
                Log.e("AudioRecordUtils", e.getMessage());
            } else {
                Log.e("AudioRecordUtils", "Unknown error occured while initializing recording");
            }
            state = State.ERROR;
        }
    }
    ...
    public void setOutputFile(String argPath) {
        try {
            ...
            if (e.getMessage() != null) {
                Log.e("setOutputFile", e.getMessage());
            } else {
                Log.e("setOutputFile", "Unknown error occured while setting output path");
            }
            state = State.ERROR;
        }
    }
    ...
    public void prepare() {
        ...
        Log.d("prepare prepare state:", state + "");
        Log.d("prepare prepare filePath:", filePath + "");
        Log.d("prepare audioRecorder.getState():", audioRecorder.getState() + "");
        ...
        try {
            if (state == State.INITIALIZING) {
                if (rUncompressed) {
                    if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED)
                        ...
                        randomAccessWriter.writeShort(
                                Short.reverseBytes((short) 1)); // AudioFormat, 1 for
                        // PCM
                        randomAccessWriter.writeShort(
                                Short.reverseBytes(nChannels)); // Number of channels,
                        // 1 for mono, 2 for
                        // stereo
                        ...
                        randomAccessWriter.writeInt(
                                Integer.reverseBytes(
                                        sRate * bSamples * nChannels / 8)); // Byte rate,
                        // SampleRate*NumberOfChannels*BitsPerSample/8
                        randomAccessWriter.writeShort(
                                Short.reverseBytes((short) (nChannels * bSamples / 8))); // Block
                        // align,
                        // NumberOfChannels*BitsPerSample/8
                        randomAccessWriter.writeShort(
                                Short.reverseBytes(bSamples)); // Bits per sample
                        ...
                        Log.e("prepare", "prepare() method called on uninitialized recorder");
                        state = State.ERROR;
                    }
                } else {
                ...
                Log.e("prepare", "prepare() method called on illegal state");
                release();
                state = State.ERROR;
            }
        } catch (Exception e) {
            if (e.getMessage() != null) {
                ...
                Log.e("prepare", "Unknown error occured in prepare()");
            }
            state = State.ERROR;
        }
    }
        ...
        if (state == State.RECORDING) {
            ...
            if ((state == State.READY) & (rUncompressed)) {
                try {
                    ...
                    Log.e("release", "I/O exception occured while closing output file");
                }
        ...
        if (state == State.RECORDING) {
            if (rUncompressed) {
                ...
                try {
                    ...
                    Log.e("stop", "I/O exception occured while closing output file");
                    state = State.ERROR;
                }
            } else {
                mediaRecorder.stop();
            ...
            Log.e("stop", "called on illegal state");
            state = State.ERROR;
        }
    }
     ...
     * <p>: 文件路径
     */
    public void recordChat(String filePath, String fileName) {
        if (!Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)) {
            return;
        }
        String file = new String(filePath + File.separator + fileName);
        // 设置输出文件
        setOutputFile(file);
        prepare();
        // 开始录音
    ...
    public void recordChat(String fileName) {
        ...
        String file =
                new String(makeFileStroagePath().getAbsolutePath() + File.separator + fileName);
        // 设置输出文件
        setOutputFile(file);
        prepare();
        // 开始录音
    ...
    public File makeFileStroagePath() {
        File file = new File(FilePathConfig.APP_AUDIO);
        try {
            if (!file.exists()) {
                file.mkdirs();
                return file;
